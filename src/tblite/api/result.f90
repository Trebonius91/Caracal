...
!subroutine get_result_wavefunction_api(verror, vres, wfn) &
!      & bind(C, name=namespace//"get_result_bond_orders")
!   type(c_ptr), value :: verror
!   type(vp_error), pointer :: error
!   type(c_ptr), value :: vres
...
!
!   if (.not.allocated(res%results)) then
!      call fatal_error(error%ptr, "Result does not contain a wavefunction")
!      return
!   end if
!
!   if (.not.allocated(res%results%wfn)) then
!      call fatal_error(error%ptr, "Result does not contain a wavefunction")
!      return
!   end if
!
!   wfn(:size(res%results%wfn)) = &
!      & reshape(res%results%wfn, [size(res%results%wfn)])
!end subroutine get_result_wavefunction_api
