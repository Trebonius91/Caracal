...
subroutine xtb_singlepoint(ctx, mol, calc, wfn, accuracy, energy, gradient, sigma, &
   ...
   if (allocated(calc%halogen)) then
      ...
      if (prlevel > 0) &
         write(84,*) label_halogen // format_string(sum(exbond), real_format) // " Eh"
   ...
   if (allocated(calc%repulsion)) then
      ...
      if (prlevel > 0) &
         write(84,*) label_repulsion // format_string(sum(erep), real_format) // " Eh"
   ...
   if (allocated(calc%dispersion)) then
      ...
      if (prlevel > 0) &
         write(84,*) label_dispersion // format_string(sum(edisp), real_format) // " Eh"
   ...
   if (allocated(calc%interactions)) then
      ...
      if (prlevel > 0) &
         write(84,*) label_other // format_string(sum(eint), real_format) // " Eh"
   ...
   if (prlevel > 0) &
      write(84,*) label_electrons // format_string(wfn%nocc, real_format) // " e"
   ...
   if (prlevel > 0) then
      write(84,*) label_cutoff // format_string(cutoff, real_format) // " bohr"
      write(84,*) ""
   end if
   ...
   if (prlevel > 0) then
      write(84,*) repeat("-", 60)
      write(84,*) "  cycle        total energy    energy error   density error"
      write(84,*) repeat("-", 60)
   end if
   do while(.not.converged .and. iscf < calc%max_iter)
      ...
      if (prlevel > 0) then
         write(84,*) format_string(iscf, "(i7)") // &
...
!
   ...
   block
      ...
      if (prlevel > 0) then
         ...
         write(84,*) "Total time for this calculation:"//repeat(" ", 1)//format_time(ttime)
      end if
      if (prlevel > 0) then
         do it = 1, size(label)
            stime = timer%get(label(it))
            if (stime <= epsilon(0.0_wp)) cycle
            write(84,*) " - "//label(it)//format_time(stime) &
