...
module aenet_potential

  use aenet_aeio,        only: PATHLEN, TYPELEN, &
                         aeio_assert_file_exists

  use aenet_geometry,    only: geo_itype_of_name

  use aenet_io,          only: io_unit

  use aenet_feedforward, only: Network,          &
                         load_Network,     &
                         del_Network,      &
                         ff_print_info

  use aenet_sfsetup,     only: Setup,            &
                         load_Setup,       &
                         del_Setup,        &
                         stp_init,         &
                         stp_final,        &
  ...
  use aenet_trainset,    only: TrnSet,           &
                         load_TrnSet_info, &
                         close_TrnSet,     &
                         ts_print_info
