...
module aenet_sfsetup

  use aenet_aeio,     only: aeio_readline, &
                      TYPELEN

  use aenet_io,       only: io_lower,      &
                      io_readval,    &
                      io_adjustl,    &
                      io_unit
  ...
  use aenet_symmfunc, only: sf_init,       &
                      sf_final,      &
                      sf_add_rad,    &
                      sf_add_ang,    &
                      sf_fingerprint
...
contains
  ...
  subroutine stp_eval(itype0, coo0, n, coo1, type1, stp, sfval, &
    ...
    do i = 1, n
       type1_loc(i) = type1(i) ! stp%ltype(type1(i))
    end do
